...
export class AmericanKennelClubService {
  ...
  async findAll(locale: string): Promise<AmericanKennelClub[]> {
  ...
  async findOne(
    id: number,
    locale: string,
  ): Promise<AmericanKennelClub | null> {
  ...
  private mapToEntity(record: any, locale: string): AmericanKennelClub {
    const entity = new AmericanKennelClub();
