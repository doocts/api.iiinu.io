...
export class CoatService {
  ...
  async findAll(locale: string): Promise<Coat[]> {
  ...
  async findOne(id: number, locale: string): Promise<Coat | null> {
  ...
  private mapToEntity(record: any, locale: string): Coat {
    const entity = new Coat();
