...
export class CanadianKennelClubService {
  ...
  async findAll(locale: string): Promise<CanadianKennelClub[]> {
  ...
  async findOne(
    id: number,
    locale: string,
  ): Promise<CanadianKennelClub | null> {
  ...
  private mapToEntity(record: any, locale: string): CanadianKennelClub {
    const entity = new CanadianKennelClub();
