...
export class NewZealandKennelClubService {
  ...
  async findAll(locale: string): Promise<NewZealandKennelClub[]> {
  ...
  async findOne(
    id: number,
    locale: string,
  ): Promise<NewZealandKennelClub | null> {
  ...
  private mapToEntity(record: any, locale: string): NewZealandKennelClub {
    const entity = new NewZealandKennelClub();
